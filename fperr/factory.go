...
// New creates a new fperr.Error with a stack
// It is recommended that `New` be called from the place where the actual
// error occurs and not do define errors.
// This is because fperr.Error is a stateful construct which also
// encapsulates the stacktrace at the time of creation.
//
// For converting generic errors to fperr.Error the recommended usage pattern
// is fperr.Wrap or fperr.Wrapf
...
// Wrap creates a new fperr.Error if the `error` that is being wrapped
// is not an fperr.Error
//
...
func Wrap(err error, options ...Option) error {
	...
	// if the error we wrapped was not an fperr,
