...
func New(ln Listener, c *Config) *Store {
	...
	dbPath := filepath.Join(c.Dir, c.ID, sqliteFile)
	...
	return &Store{
		ln:              ln,
		raftDir:         filepath.Join(c.Dir, c.ID),
		peersPath:       filepath.Join(c.Dir, c.ID, peersPath),
		peersInfoPath:   filepath.Join(c.Dir, c.ID, peersInfoPath),
		raftID:          c.ID,
		dbConf:          c.DBConf,
		dbPath:          dbPath,
...
func (s *Store) Open() (retErr error) {
	...
	s.logger.Printf("observer registering")
	s.raft.RegisterObserver(s.observer)
	s.logger.Printf("observer registered")
	s.logger.Printf("observer observing")
	s.observerClose, s.observerDone = s.observe()
	s.logger.Printf("observer observed")
