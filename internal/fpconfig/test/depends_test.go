...
func TestImplicitDependsIndex(t *testing.T) {
	...
	pipelines, _, err := pipeparser.LoadPipelines(context.TODO(), "./test_pipelines/depends.fp")
...
func TestImplicitQueryDepends(t *testing.T) {
	...
	pipelines, _, err := pipeparser.LoadPipelines(context.TODO(), "./test_pipelines/query_depends.fp")
	assert.Nil(err, "error found")
